...
def catalogList():
    cats = dataSQL.getAllCategories()
    items = dataSQL.getAllRecentItems()
    current_user = dataSQL.getUser(login_session.get('user_id'))
...
def category_items(category_name):
    category = dataSQL.getCategoryByName(category_name)
    items = dataSQL.getCategoryItems(category.id)
    current_user = dataSQL.getUser(login_session.get('user_id'))
...
def category_item_details(category_name, item_name):
    category = dataSQL.getCategoryByName(category_name)
    item = dataSQL.getItemByNameCat(item_name, category.id)
    current_user = dataSQL.getUser(login_session.get('user_id'))
...
def category_item_add():
    if request.method == 'POST':
        ...
        category = dataSQL.getCategoryByName(ct)
        cats = dataSQL.getAllCategories()
        current_user = dataSQL.getUser(login_session.get('user_id'))
...
def category_item_edit(item_name):
    if request.method == 'POST':
        ...
        category = dataSQL.getCategoryByName(ct_name)
        old_item = dataSQL.getItem(id)
        ...
        if old_item.name == nm and old_item.desc == ds and old_item.category.name == ct_name:
            ...
            # if updates, save
            dataSQL.updateItem(old_name=old_item.name, name=nm, desc=ds, category=category)
            return redirect(url_for('catalogList'))
    else:
        current_user = dataSQL.getUser(login_session.get('user_id'))
        cats = dataSQL.getAllCategories()
        item = dataSQL.getItemByName(item_name=item_name)
...
def category_json():
    cats = dataSQL.getAllCategories()
    return jsonify(Category=[c.category_serialize for c in cats])


@app.route('/catalog/items.json')
def items_json():
    items = dataSQL.getAllItems()
    return jsonify(Items=[i.items_serialize for i in items])


@app.route('/catalog/<string:item_name>/item.json')
def item_json(item_name):
    item = dataSQL.getItemByName(item_name)
    if item:
        return jsonify(item.items_serialize)
    else:
...
def catalog_items_json(category_name):
    cat = dataSQL.getCategoryByName(category_name)
    if cat:
        return jsonify(cat.category_serialize)
    else:
...
def gconnect():
    ...
    user_id = dataSQL.getUserId(user_email=em)
    # print 'user_id = %s' % user_id
    if user_id is None:
        user_id = dataSQL.addNewUser(login_session)
